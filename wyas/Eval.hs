...
fileToEvalForm input =
    either (throw . PError . show)
            evalBody $ readExprFile input

runParseTest :: T.Text -> T.Text
runParseTest input =
    either (T.pack . show)
           (T.pack . show) $ readExpr input

runASTinEnv :: EnvCtx -> Eval b -> IO b
runASTinEnv code action =
    runResourceT $ runReaderT (unEval action) code
...
applyLambda expr params args = do
    ...
    let env' = Map.fromList (Prelude.zipWith
        (\x y -> (extractVar x,y) ) params argEval) <> env
...
eval (List [Atom "let", List pairs, expr]) = do
    ...
    let env' = Map.fromList
        (Prelude.zipWith (\x y -> (extractVar x,y)) atoms vals) <> env
...
eval (List [Atom "define", varExpr, expr]) = do
    ...
    evalVar <- eval expr
    env     <- ask
    let envFn = const $ Map.insert (extractVar varAtom) evalVar env
